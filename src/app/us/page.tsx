...
export const metadata = {
  title: '미국 뉴스 - LiveNews',
  description: '미국 주요 뉴스를 실시간으로 확인하세요',
};
...
async function getArticles(page: number) {
  ...
  try {
    return await getCached(`us:${page}`, 60, async () => {
      const [articles, total] = await Promise.all([
        ...
        prisma.article.count({ where: { country: 'us', isActive: true } }),
      ]);
...
export default async function USPage({
  ...
  const subcategories = COUNTRY_SUBCATEGORIES.us;

  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
      <div className="flex items-center gap-3 mb-6">
        <h1 className="text-2xl font-bold">미국 뉴스</h1>
      ...
      <Pagination currentPage={page} totalPages={totalPages} basePath="/us" />
    </div>
  );
}
