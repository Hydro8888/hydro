...
export function BottomNav() {
  const p = usePathname();
  return (
    <nav className="fixed inset-x-0 bottom-0 z-50 bg-white border-t border-[#ddd] md:hidden">
        ...
        {NAV.map(({ href, label, Icon }) => {
          ...
          return (
            <Link key={href} href={href} className="flex flex-col items-center gap-0.5 py-1 px-3">
              ...
              <span className={`text-[10px] ${on ? 'font-bold text-[#1E3A5F]' : 'text-[#999]'}`}>{label}</span>
            </Link>
          );
        })}
      </div>
