              ...
              <div className="flex min-w-0 items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex min-w-0 items-center gap-2">
                    <span className="flex h-6 min-w-6 shrink-0 items-center justify-center rounded-full border border-current/25 bg-background/35 px-1 text-[11px] font-black">
                      {step.count ?? String(index + 1).padStart(2, "0")}
                    </span>
                    <h3 className="min-w-0 truncate text-sm font-black text-foreground">{step.title}</h3>
                  </div>
                  {step.subtitle ? (
                    <p className="mt-1 line-clamp-2 text-xs leading-5 text-muted-foreground">{step.subtitle}</p>
                  ) : null}
                </div>
              </div>
              {step.result ? (
                <div className="mt-3 rounded-md border border-success/20 bg-success/10 px-3 py-2">
                  <p className="text-[10px] font-black uppercase tracking-[0.14em] text-success">결과</p>
                  <p className="mt-0.5 break-keep text-xs leading-5 text-foreground/85">{step.result}</p>
                </div>
              ) : null}
              {step.detail ? (
                <p className="mt-3 rounded-md border border-current/15 bg-background/25 px-3 py-2 text-xs leading-5">
                  {step.detail}
                </p>
              ) : null}
              {isActionable ? (
                <span className="mt-3 inline-flex min-h-8 w-full items-center justify-center rounded-md border border-current/20 bg-background/25 px-3 text-xs font-bold text-foreground transition group-hover:border-primary/40 group-hover:text-primary">
