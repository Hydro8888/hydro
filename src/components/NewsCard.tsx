    ...
    <article className="group flex flex-col overflow-hidden rounded-card bg-surface-card border border-border-muted hover:border-border hover:shadow-elevated transition-all duration-300">
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <div className="mb-2 flex flex-wrap items-center gap-2 text-caption">
          {catLabel && (
            <span className={cn('border-l-2 px-2 py-0.5 rounded-badge', catStyle.border, catStyle.text, catStyle.bg)}>
              {catLabel}
            </span>
          )}
        ...
        {/* Summary */}
        {summary && (
          <p className="mb-3 text-body-md text-text-secondary line-clamp-2">
            {summary}
          </p>
        )}
