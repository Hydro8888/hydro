...
export function Sidebar({ selectedRegion, selectedJobType, onRegionChange, onJobTypeChange }: Props) {
  const pill = (on: boolean) => `rounded-lg px-2.5 py-1 text-xs transition ${on ? 'bg-[#e85d8a] text-white' : 'bg-[#1e142a] text-[#9a8aa8] hover:text-white'}`;
  return (
    <aside className="card hidden w-56 shrink-0 p-4 lg:block">
      <h3 className="mb-2 text-xs font-semibold text-[#d4a76a]">지역</h3>
      ...
      <h3 className="mb-2 text-xs font-semibold text-[#d4a76a]">업종</h3>
      <div className="mb-4 flex flex-wrap gap-1.5">
      ...
      <h3 className="mb-2 text-xs font-semibold text-[#d4a76a]">정렬</h3>
      <select className="w-full rounded-lg border border-[#2a1e3a] bg-[#1e142a] px-3 py-2 text-sm text-[#9a8aa8] outline-none focus:border-[#e85d8a]">
