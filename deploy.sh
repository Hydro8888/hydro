...
#=============================================================================
# AI Portal Pro - 원스톱 배포 스크립트
# 서버: Ubuntu 211.198.54.207 (SSH 포트 2222)
# 경로: http://211.198.54.207/ai-portal/
# 포트: 3010 (PM2 관리)
#=============================================================================
...
if [ -z "${XAI_API_KEY}" ]; then
  ...
  echo "사용법: XAI_API_KEY=your-key-here bash deploy.sh"
  exit 1
fi
...
echo "============================================"
echo "  AI Portal Pro 배포 시작"
echo "  서버: ${SERVER_IP}:${SSH_PORT}"
echo "  경로: /ai-portal/"
echo "  포트: ${APP_PORT}"
echo "============================================"

#=============================================================================
# Phase 1: SSH 서버에 원격 배포
#=============================================================================
echo ""
echo "[Phase 1] 서버 원격 배포 시작..."
echo ""

ssh -o StrictHostKeyChecking=no -p ${SSH_PORT} ${SSH_TARGET} bash << REMOTE_DEPLOY

set -e

echo "=== [1/9] Node.js / pnpm 확인 ==="
...
echo ""
echo "=== [3/9] .env 파일 생성 ==="
cat > ${DEPLOY_DIR}/apps/web/.env.local << 'ENVEOF'
...
# XAI_API_KEY 실제 값 치환
sed -i "s|XAI_API_KEY=\\\${XAI_API_KEY}|XAI_API_KEY=${XAI_API_KEY}|g" ${DEPLOY_DIR}/apps/web/.env.local
...
if curl -s -o /dev/null -w "%{http_code}" http://localhost:${APP_PORT}/ai-portal/ | grep -q "200\|301\|302\|304"; then
  ...
  echo "경고: 앱 응답 확인 실패. PM2 로그를 확인하세요: pm2 logs ${APP_NAME}"
fi
...
NGINX_BLOCK
    ...
    # nginx 설정의 마지막 } 앞에 location 블록 삽입
    sudo docker cp \${NGINX_CONF} /tmp/nginx-original.conf 2>/dev/null || \
      sudo docker exec ${NGINX_CONTAINER} cat \${NGINX_CONF} > /tmp/nginx-original.conf

    # 마지막 닫는 중괄호 } 앞에 삽입
    sudo sed -i '/^}$/i\\' /tmp/nginx-original.conf
...
echo "  접속 URL:    http://${SERVER_IP}/ai-portal/"
echo "  직접 접속:   http://localhost:${APP_PORT}/ai-portal/"
echo ""
pm2 status
...
echo "============================================"
echo "  원격 배포 완료!"
echo "  접속: http://${SERVER_IP}/ai-portal/"
echo "============================================"
