...
export default function ReviewsPage() {
  return (
    <div className="mx-auto max-w-[640px] px-4 py-5">
      <h1 className="text-xl font-bold mb-1">광고후기</h1>
      <p className="text-sm text-[#999] mb-5">실제 근무 경험을 공유합니다</p>
      <div className="space-y-3">
        {DATA.map(r => (
          <div key={r.id} className="card p-4">
            <div className="flex items-start justify-between">
              <div><h3 className="text-base font-bold">{r.store}</h3><p className="text-xs text-[#999]">{r.region}</p></div>
              <div className="flex gap-0.5">{[1,2,3,4,5].map(i => <Star key={i} className={`h-4 w-4 ${i <= r.rating ? 'fill-[#C9A961] text-[#C9A961]' : 'text-[#e0e0e0]'}`} />)}</div>
            </div>
            <p className="mt-3 text-sm text-[#666]">{r.text}</p>
            <div className="mt-3 flex items-center justify-between text-xs text-[#999]">
              ...
              <button className="flex items-center gap-1 hover:text-[#C9A961]"><ThumbsUp className="h-3.5 w-3.5" /> {r.likes}</button>
            </div>
          </div>
        ))}
      </div>
